...
namespace RequisicionesApi.Models
{
    public class ImputacionValRepository : IImputacionValRepository
    {
        ...
        public async Task<IEnumerable<ImputacionVal>> GetAllAsync()
        {
            ...
            var command = new SqlCommand("SELECT impIdImp, CAST(impIdSArea AS VARCHAR) AS impIdSArea, impvValor FROM tblImpVal", connection);
        ...
        {
            using var connection = new SqlConnection(_connectionString);
            var command = new SqlCommand("SELECT impIdImp, impIdSArea, impvValor FROM tblImpVal WHERE impIdImp = @Id", connection);
        ...
        public async Task CreateAsync(ImputacionVal entity)
        {
            ...
            var command = new SqlCommand(
                "INSERT INTO tblImpVal (impIdImp, impIdSArea, impvValor) VALUES (@ImpIdImp, @ImpIdSArea, @ImpvValor)", connection);
        ...
        public async Task UpdateAsync(ImputacionVal entity)
        {
            using var connection = new SqlConnection(_connectionString);
            var command = new SqlCommand(
                "UPDATE tblImpVal SET impIdSArea = @ImpIdSArea, impvValor = @ImpvValor WHERE impIdImp = @ImpIdImp", connection);
        ...
        public async Task<IEnumerable<ImputacionValDetalle>> GetAllDetalleAsync()
        {
            ...
            using var connection = new SqlConnection(_connectionString);
            var query = @"
            SELECT i.impIdImp, i.impNombre, sarSAreaC, sarIdArea, a.arNombre,
                   iv.impIdSArea, sa.arNombre AS Subarea, impvValor
